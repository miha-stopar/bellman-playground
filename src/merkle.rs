    ...
    H: MerkleHasher<E>,
{
    ...
    pub fn check_inclusion<CS: ConstraintSystem<E>>(
        ...
        for (i, e) in self.witness.iter().enumerate() {
            ...
            acc = self.hasher.hash_couple(
                cs.namespace(|| "hash couple"),
                &xl,
                &xr,
            )?;
        }
...
mod test {
    ...
    mod single {
            ...
            E: JubjubEngine,
        {
            fn alloc<CS: ConstraintSystem<E>>(
                ...
                let m =
                    num::AllocatedNum::alloc(cs.namespace(|| "root"), || {
    ...
    mod bignat_hasher {
            ...
            H: CircuitHasher<E = E> + Hasher<F = E::Fr>,
        {
            fn hash_to_leaf<CS: ConstraintSystem<E>>(
                ...
                self.hasher.allocate_hash(
                    cs.namespace(|| "hash_to_leaf"),
                    raw.as_slice(),
                )
            }
            fn hash_couple<CS: ConstraintSystem<E>>(
    ...
    fn test_merkle_circuit_bls12() {
        ...
        let proof =
            create_random_proof(instance, &circuit_parameters, &mut rng)
                .unwrap();
        let is_valid =
